...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (gamma, beta, bn_param, x_bar, x, sample_mean, sample_var)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    #############################################################################
    gamma, beta, bn_param, x_bar, x, sample_mean, sample_var = cache
    eps = bn_param.get('eps', 1e-5)
    dgamma = np.sum(dout * x_bar, axis=0)
    dbeta = np.sum(dout, axis=0)
    dvar = np.sum(dout * gamma * (x - sample_mean) * (-0.5) * ((sample_var + eps) ** (-1.5)), axis=0)
    dmean = np.sum(dout * gamma * (-1.0) / np.sqrt(sample_var + eps), axis=0) - dvar * 2.0 / x_bar.shape[0] * np.sum(
        x - sample_mean, axis=0)
    dx = dout * gamma / np.sqrt(sample_var + eps) + dvar * 2 * (x - sample_mean) / x_bar.shape[0] + dmean / x_bar.shape[
        0]
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def max_pool_forward_naive(x, pool_param):
    ...
    N, C, H, W = x.shape
    # HH=min(H,HH)
    # WW=min(W,WW) #not need anymore !need to add to avoid negative number for Hp or Wp
    Hp = 1 + (H - HH) / stride
    Wp = 1 + (W - WW) / stride
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    #############################################################################
    N, C, H, W = x.shape
    x_tran = x.transpose(1, 0, 2, 3).reshape(C, N * H * W).T
    out_tran, cache = batchnorm_forward(x_tran, gamma, beta, bn_param)
    out = out_tran.T.reshape(C, N, H, W).transpose(1, 0, 2, 3)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    #############################################################################
    N, C, H, W = dout.shape
    dout_tran = dout.transpose(1, 0, 2, 3).reshape(C, N * H * W).T
    dx_tran, dgamma, dbeta = batchnorm_backward(dout_tran, cache)
    dx = dx_tran.T.reshape(C, N, H, W).transpose(1, 0, 2, 3)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
#     N = x.shape[0]
#     print N
#     loss = -np.sum(np.log(probs[0, y[0]])) / N
#     dx = probs.copy()
#     dx[0, y[0]] -= 1
#     dx /= N
#     dx1 = np.array([dx[0][0:x1.size]])
#     dx2 = np.array([dx[0][x1.size:dx[0].size]])
#     print dx1.shape
#     print dx2.shape
#     print loss
#     return loss, dx1, dx2
...
def cosine_loss(x1, x2, y):
    """
    Computes the loss and gradient for cosine similarity.

    Inputs:
    - x1: Input data, of shape (N, C) where x1[i, j] is the value of jth feature
      for the ith input.
    - x2: Input data, of shape (N, C) where x2[i, j] is the value of jth feature
      for the ith input.
    - y: Vector of labels, of shape (N,) where y[i] is the similarity for x1[i] and x2[i]
      y = 1 or 0

    Returns a tuple of:
    - loss: Scalar giving the loss
    - dx1,dx2: Gradient of the loss with respect to x1,x2
    """
    x1_norm = np.sqrt(np.sum(x1 ** 2, axis=1, keepdims=True))
    x2_norm = np.sqrt(np.sum(x2 ** 2, axis=1, keepdims=True))
    prod = np.sum(x1 * x2, axis=1, keepdims=True)
    ...
    # print "loss"
    # print loss
    dloss = -1.0 * (y.reshape(y.shape[0], 1) - cosine_sim) / y.shape[0]
    # print "dloss"
    # print dloss
    dx1 = dloss * (x2 * x1_norm * x2_norm - prod * x2_norm * x1 / x1_norm) / (x1_norm * x2_norm) ** 2
    # print "dx1"
    # print dx1
    dx2 = dloss * (x1 * x1_norm * x2_norm - prod * x1_norm * x2 / x2_norm) / (x1_norm * x2_norm) ** 2
    # print "dx2"
    # print dx2
    return loss, dx1, dx2
